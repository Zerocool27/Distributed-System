// Generated on 2018-03-25 using generator-angular-material-fullstack 0.1.2
'use strict';
...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        shipit: {
            options: {
                workspace: '/Users/fatih/Projects/ds_project/dist',
                deployTo: '/var/www/searchapp',
                repositoryUrl: 'git@github.com:Zerocool27/Distributed-System.git',
                ignores: ['.git', 'node_modules'],
                keepReleases: 2
            },
            production: {
                servers: ['ubuntu@ec2-54-186-200-154.us-west-2.compute.amazonaws.com',
                    'ubuntu@ec2-34-208-124-176.us-west-2.compute.amazonaws.com'
                ]
            }
        }
    });
    ...
    grunt.registerTask('remote:install', function() {
        ...
        grunt.shipit.remote('cd ' + current + ' && npm init && npm rebuild', done);
        // grunt.shipit.remote('cd /var/www/tv8/current && npm install', this.async());
    });

    grunt.registerTask('remote:restart', function() {
        // grunt.shipit.remote('cd /var/www/tv8/current && pm2 restart processes.json', this.async());
        var done = this.async();
        var current = grunt.config('shipit.options.deployTo') + '/current';
        grunt.shipit.remote('cd ' + current + ' && pm2 restart process.json ', done);
        // grunt.shipit.remote('cd ' + current + ' && pm2 restart process_staging.json ', done);
